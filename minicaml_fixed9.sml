(* ===== Compatibility layer for SML/NJ Basis ===== *)
structure Compat = struct
  (* input: read exactly n characters (or fewer at EOF) *)
  fun input (ins: TextIO.instream, n: int) : TextIO.vector = TextIO.inputN (ins, n)

  (* lookahead: return one-character string or "" at EOF *)
  fun lookahead (ins: TextIO.instream) : string =
  ...
  fun flush_out (os: TextIO.outstream) : unit = TextIO.flushOut os

  (* 1-char string ord/chr helpers *)
  fun ord1 (s:string) : int = Char.ord(String.sub (s, 0))
  fun chr1 (i:int)    : string = String.str (Char.chr i)
end

(* Abbrev wrappers to minimize downstream edits *)
val std_in  = Compat.std_in
val std_out = Compat.std_out
val flush_out = Compat.flush_out
fun input x = Compat.input x
fun lookahead ins = Compat.lookahead ins
fun ord1 s = Compat.ord1 s
fun chr1 i = Compat.chr1 i

(* List.nth alias *)
fun nth (xs, i) = List.nth (xs, i)

(* Printing helpers *)
fun printInt (n:int)   = TextIO.print (Int.toString n)
fun printBool (b:bool) = TextIO.print (if b then "true" else "false")
...
Def ( true, fac, Function ( ( Motif_number 0, Number 1 ) | ( Motif_variable "x", Application ( Variable "*", Pair ( Application ( Variable "fac", Application ( Variable "-", Pair ( Variable "x", Number 1 ) ) ), Variable "x" ) ) ) ) )
fac : (int -> int) = <fun>
## fac;
Expr ( Variable "fac" )
- : (int -> int) = <fun>
## fac 5;
Expr ( Application ( Variable "fac", Number 5 ) )
- : int = 120
## fac 11;
Expr ( Application ( Variable "fac", Number 11 ) )
- : int = 39916800
## fac 12;
Expr ( Application ( Variable "fac", Number 12 ) )
- : int = 479001600
## fac 13;
Expr ( Application ( Variable "fac", Number 13 ) )
- : int = 6227020800
## fac 15;
Expr ( Application ( Variable "fac", Number 15 ) )
- : int = 1307674368000
## fac 20;
Expr ( Application ( Variable "fac", Number 20 ) )
...
fun integer ISTREAM i =
	...
	integer ISTREAM (10*i+ord1(input(ISTREAM,1))-ord1("0"))
    else i
...
and print_definition x = case x of
    Def(b,s,e) => ( print("Def ( "); printBool b; print(", "); print s;
...
|   Motif_boolean b => ( print "Motif_boolean "; printBool b )
|   Motif_number n => ( print "Motif_number "; printInt n )
...
and print_value x = case x of
    Val_bool b => ( print "Val_bool "; printBool b )
...
val Val_env_initial = [
    ...
    ("write_int", prim1 code_number (fn (x:int) => (printInt x; print "\n"; 0))
					code_number_fn ),
...
fun schema_trivial (ty : simple_type) : types_schema = { Parameters = [], Bodys = ty };
val type_arithmetic : types_schema = schema_trivial
    (type_arrow (type_product type_int type_int) type_int)
and type_comparison : types_schema = schema_trivial
    (type_arrow (type_product type_int type_int) type_bool)
val type_env_initial : (string * types_schema) list = [
...
and print_variable_of_type (vt as (ref { Index=i, Value=v }) ) =
    ( print "ref { Index = "; printInt i; print ", Value = ";
...
fun display_var vv = ( print "'";
	...
	Not_found =>
	    let val name = chr1(ord1("a") + (!count_of_variables)) in
