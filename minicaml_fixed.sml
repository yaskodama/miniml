...
(* --- SML/NJ Basis compatibility shim for this MINICAML port --- *)
structure Compat = struct
  (* Return a string of length n from the stream. *)
  fun input (ins, n) = TextIO.inputN (ins, n)
  (* Peek one character and return it as a length-1 string, or empty string on EOF. *)
  fun lookahead ins =
  ...
  fun flush_out out = TextIO.flushOut out
  (* ord/chr on 1-char strings to match this codebase's expectations *)
  fun ord1 s = Char.ord1(String.sub (s, 0))
  fun chr1 i = String.str (Char.chr i)
end
open Compat
(* --- end shim --- *)

(* extra helpers for SML/NJ Basis *)
